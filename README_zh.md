...
- 左手空着右键机器:从机器中取出核心
- 左手拿着核心右键机器:将核心插入机器
- 潜行 + 右键机器:拆除机器并将机器和核心返还到背包
- 潜行 + 右键虚空发电机:拆除虚空发电机并将其返还到背包
...
### 虚空发电机(Void Generator)

虚空发电机消耗虚空流体，将其转换为 FE 能量并向外输出。
...
|---|---|
| 底面(DOWN) | 虚空流体输入 |
| 其余五个面 | 能量输出(FE)，可通过扳手配置模式单独开关 |

**工作条件:**
1. 底面有虚空流体输入
2. 至少一个面处于开启状态

**转换比:** 默认 1 mB 虚空流体 = 10 FE（可在配置中修改）

**侧面速率:** 每个面可单独设置输出速率（FE/t），用扳手配置模式右键切换开关，潜行右键打开速率输入界面。

**红石控制:** 每次接收到红石信号（从无到有）时，切换所有输出面的开关状态（全开 ↔ 全关）。

**HUD 信息:** 对准虚空发电机时显示能量储量、所需消耗（虚空流体/s 和 /t）、运行状态、虚空储罐存量，以及各输出面的速率。

**合成:** 铁锭 + 红石 + 销毁核心 + 无限核心 + 虚空桶（3×3 合成）

---

### 强化虚空方块(Reinforced Void Block)

强化虚空方块是一种极其坚固的建筑方块，对虚空流体完全免疫。

- **硬度:** 100（黑曜石为 50），需要钻石镐开采
- **爆炸抗性:** 2400（黑曜石为 1200）
- **虚空免疫:** 虚空流体无法吞噬或扩散到强化虚空方块，可用于构建安全隔离区域
...
- **能量**:当前储量 / 最大容量
- **耗电 / 所需消耗**:两种机器显示耗电（FE/s 和 FE/t），虚空发电机显示虚空流体消耗（mB/s 和 mB/t）
